...
case class SpecialForm(
  name: String, arity: Int, varg: Boolean, body: (Seq[SExpr]) => (SExpr, Env)) extends Proc
...
case class BuiltinFunc (
  name: Option[String], arity: Int, varg: Boolean, body: (Seq[SExpr]) => (SExpr, Env)) extends Func
